...
public contract sets over t determines e is {
...
public implementation equality over (set of %v where equality over %v) is {
  fun X=Y is __set_equal(X,Y);
  fun hashCode(S) is __set_hash(S)
}

public implementation sizeable over set of %t is {
  fun isEmpty(M) is __set_empty(M);
  fun size(R) is integer(__set_size(R));
}

public implementation pPrint over set of %v where pPrint over %v is {
...
public implementation updateable over set of %v determines %v is {
...
public implementation sequence over set of %v determines %v is {
...
public implementation iterable over set of %v determines %v is {
  fun _iterate(M,F,S) is __set_iterate(M,F,S);
}

public implementation mappable over set is {
  fun map(F,S) is __set_map(F,S)
}

public implementation foldable over set of %v determines %v is {
...
public implementation concatenate over set of %v is {
  fun M1++M2 is __set_union(M1,M2);
}

public implementation sets over set of %t determines %t is {
