...
public class WffInfo implements WffCond
{
  private final WffBuildOp val;

  public WffInfo(WffBuildOp val)
  {
    this.val = val;
  }
  ...
  public applyMode satisfied(IAbstract env[], Location loc, WffEngine engine)
  {
    ...
    if (tgt instanceof StringLiteral)
      engine.reportInfo(((StringLiteral) tgt).getLit(), loc);
    else
      engine.reportInfo(tgt.toString(), loc);
  ...
  {
    disp.append(StandardNames.INFO);
    disp.append("(");
