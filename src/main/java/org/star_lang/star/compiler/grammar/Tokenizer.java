...
import static org.star_lang.star.compiler.grammar.Token.TokenType.*;
...
public class Tokenizer {
  ...
  private void checkForMultiTokens() {
    ...
    if (token.getType() == TokenType.identifier) {
      ...
      if (multis != null) {
        multiLoop:
        for (Pair<String[], String> multi : multis) {
  ...
  public static boolean isIdentifierStart(int cp) {
    return !Character.isDigit(cp) && isIdentifierChar(cp);
  }
  ...
  private Token reportToken(int ix) {
    return reportToken(ix, false);
  }
