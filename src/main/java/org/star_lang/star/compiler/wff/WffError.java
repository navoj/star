...
public class WffError implements WffCond
{
  private final WffBuildOp val;

  public WffError(WffBuildOp val)
  {
    this.val = val;
  }
  ...
  public applyMode satisfied(IAbstract env[], Location loc, WffEngine engine)
  {
    ...
    if (tgt instanceof StringLiteral)
      engine.reportError(((StringLiteral) tgt).getLit(), loc);
    else
      engine.reportError(tgt.toString(), loc);
  ...
  {
    disp.append(StandardNames.WFF_ERROR);
    disp.append("(");
