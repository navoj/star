...
public type IterState of t is NoneFound or NoMore(t) or ContinueWith(t) or AbortIter(exception);
...
public contract iterable over s determines e is {
  _iterate has type for all r such that (s,(e,IterState of r)=>IterState of r,IterState of r) => IterState of r;
}

public contract indexed_iterable over s determines (k,v) is {
  _ixiterate has type for all r such that (s,(k,v,IterState of r)=>IterState of r,IterState of r) => IterState of r;
}

public type _possible of t is _impossible or _possible(t);

public fun _optionIterState(NoMore(X)) is some(X)
 |  _optionIterState(ContinueWith(X)) is some(X)
 |  _optionIterState(NoneFound) is none

public fun _checkIterState(NoMore(X),_) is X
 |  _checkIterState(ContinueWith(X),_) is X
 |  _checkIterState(NoneFound,D) is D()

public fun _negate(NoMore(true),A,B) is A()
 |  _negate(ContinueWith(true),A,B) is A()
 |  _negate(_,A,B) default is B()

public fun _otherwise(NoneFound,F) is F()
 |  _otherwise(ContinueWith(_empty()),F) is F()
 |  _otherwise(St,_) default is St

public fun _project_0_2((L,_)) is L;

-- We have to put this here because we need to import strings

public implementation iterable over string determines integer is {
  fun _iterate(M,F,S) is __string_iter(M,F,S)
}

public implementation indexed_iterable over string determines (integer,integer) is {
  fun _ixiterate(string(Str),F,S) is __string_ix_iterate(Str,F,S)
}

public contract grouping over coll determines (m,k,v) where indexable over m of (k,coll of v) determines (k,coll of v) is {
  (group by) has type ((coll of v), (v)=>k) => m of (k,coll of v)
}

public implementation (computation) over IterState determines exception is {
