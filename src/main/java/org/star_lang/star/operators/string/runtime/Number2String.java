...
public abstract class Number2String {
   ...
   * 
   * <code>S?(N|,)*.N+ESN+</code> where S is a sign spec, N is a number spec and
   * . is the period.
   * 
   * @param val
   * @param format
  ...
  public static String format(double val, String format) throws EvaluationException {
    ...
    if (ePos < 0) {
      ...
      if (exp10 > beforePeriod || exp10 + afterPeriod < -precision)
        throw new EvaluationException(StringUtils.msg("cannot format ", val, " using ", format));
      else if (exp10 < 0)
        digits = digits.substring(0, precision + exp10);
