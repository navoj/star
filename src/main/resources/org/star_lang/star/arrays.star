...
public implementation equality over list of %t where equality over %t is {
...
public implementation sizeable over list of %t is {
  fun isEmpty(L) is __array_empty(L);
  fun size(L) is integer(__array_size(L));
}

public implementation pPrint over list of %t where pPrint over %t is {
  fun ppDisp(L) is sequenceDisplay("list",L);
}

public implementation sequence over list of %e determines %e is {
...
public implementation concatenate over list of %t is {
  fun L++R is __array_concatenate(L,R);
}

public implementation indexable over list of %e determines (integer,%e) is {
...
public implementation sliceable over list of %e determines integer is {
...
public implementation iterable over list of %e determines %e is {
  fun _iterate(R,F,S) is __array_iterate(R,F,S);
}

public implementation indexed_iterable over list of %e determines (integer,%e) is {
  fun _ixiterate(M,F,S) is __array_ix_iterate(M,F,S);
}

public implementation updateable over list of %t determines %t is {
...
public implementation sorting over list of %t determines %t is {
  fun sort(L,C) is __array_sort(L,C);
}

public implementation reversible over list of %t is {
  fun reverse(A) is __array_reverse(A);
}

public implementation sets over list of %e determines %e where equality over %e is {
...
public implementation mappable over list is {
  fun map(F,A) is __array_map(A,F);
}

public implementation filterable over list of %t determines %t is {
  fun filter(P,A) is __array_filter(A,P)
}

public implementation foldable over list of %e determines %e is {
...
public implementation grouping over list determines (dictionary, %k,%v) is {
