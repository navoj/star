...
public abstract class Abstract
{
  ...
  public static int getChar(IAbstract term)
  {
    assert term instanceof CharLiteral;
    return ((CharLiteral) term).getLit();
  }
  ...
  public static CharLiteral newChar(Location loc, int ch)
  {
    return new CharLiteral(loc, ch);
  }
  ...
  {
    return term instanceof Apply && Abstract.isName(((Apply) term).getOperator(), op)
        && ((Apply) term).arity() == arity;
  }
