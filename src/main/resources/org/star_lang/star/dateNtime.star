...
public type date is date(long_) or never;

public implementation coercion over (date,string) is {
  fun coerce(date(D)) is string(__date_string(D))
   |  coerce(never) is "never"
}

public implementation coercion over (string,date) is {
  fun coerce(string(S)) where __string_date(S) has value long(D) is date(D)
   |  coerce(_) default is never
}

public implementation coercion over (date, long) is {
  fun coerce(date(D)) is long(D)
   |  coerce(never) is nonLong
}

public implementation coercion over (long,date) is {
  fun coerce(long(D)) is date(D)
   |  coerce(nonLong) is never
}

public implementation formatting over date is {
  fun _format(date(D),string(F)) is ppStr(string(__format_date(D,F)));
}

public fun parse_date(string(S),string(F)) is date(__parse_date(S,F));

public fun format_date(date(D),string(F)) is string(__format_date(D,F))
 |  format_date(never,_) is "never"

public implementation pPrint over date is {
  fun ppDisp(D) is ppStr(D as string)
}

public implementation comparable over date is {
...
public implementation largeSmall over date is {
  def largest is date(0x7fffffffffffffffL_)
  def smallest is date(0x0L_)
}

public fun now() is date(_now());

public fun today() is date(_today());

public fun timeDiff(date(F),date(T)) is long(__long_minus(F,T))
 |  timeDiff(never,_) is nonLong
 |  timeDiff(_,never) is nonLong

public fun timeDelta(date(T),long(D)) is date(__long_plus(T,D))
